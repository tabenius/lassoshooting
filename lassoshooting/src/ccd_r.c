...
SEXP ccd(SEXP args) {
  ...
  if (!givenXtX && Xm != -1 && Xn != -1) { 
    ...
    F77_CALL(dgemm)("T","N",&Xn,&Xn,&Xm, &one,X,&Xm, X,&Xm, &zero,params.XtX,&Xn);  // X'X -> XtX
  } 
  else if (givenXtX) {
  ...
  if (params.trace > 3) {
    for (int j=0; j < p; j++)
      REprintf("X'y_(%d,1)=%.4f\n",j,params.Xty[j]);
  } else if (params.trace > 2) {
    REprintf("X'y_(1,1)=%.4f\n",params.Xty[0]);
    REprintf("X'y_(2,1)=%.4f\n",params.Xty[1]);
    REprintf("X'y_(3,1)=%.4f\n",params.Xty[2]);
  }
